...
import { useFurnitureModalStore, useFurnitureStore } from "../model/store";
...
const InventoryListItem: React.FC<InventoryListItemProps> = ({ item }) => {
  ...
  const handleFurniture = (item: FurnitureItem) => {
    if (item.isOwned) {
      updateFurniture(item.furnitureId, { isPlaced: !item.isPlaced });
  ...
  return (
    <div
      onClick={() => handleFurniture(item)}
      key={item.furnitureId}
      className={`relative ${
        item.isOwned ? "bg-[#F5F5F5] rounded-[20px] border-1 border-[#DEDEDE] " : ""
      } flex flex-col items-center px-6 py-5.5 cursor-pointer  `}
    >
      <Image
        className={`${item.isPlaced ? "visible absolute top-3.5 right-3.75" : "hidden"}`}
